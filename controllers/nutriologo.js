...
const solicitarReagendacion = async (req, res = response) => {
    ...
    try {
        ...
        const { nombre } = await Nutriologo.findById(id_emisor);

        let cliente = await Cliente.findById(id_paciente);
        if(!cliente) {
            const clientes = await Cliente.find();
            for await (const user of clientes) {
                if(user.extra1) {
                    if(String(user.extra1) == id_paciente)
                    cliente = user;
                    break;
                }
                if(user.extra2) {
                    if(String(user.extra2) == id_paciente)
                    cliente = user;
                    break;
                }
            }
        ...
        const notificacion = new Notificacion('Tienes una nueva solicitud de reagendación de ' + nombre);
    ...
    try {
        ...
        for (let i = 0; i < 30; i++) {
            if(isSameDay(nutriologo.fechaDisponible[i].date[reagendacion.horaAntes], servicio.fecha_cita)){
                nutriologo.fechaDisponible[i].hora[reagendacion.horaAntes] = true;
                console.log(nutriologo.fechaDisponible[i].hora[reagendacion.horaAntes]);
                break;
            }
        }
